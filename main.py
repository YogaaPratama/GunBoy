...
class Game(): 
    def  __init__ (self):
        ...
        pygame.display.set_caption("1v1 GunBoy") #judul window
        self.run = True #variabel untuk mengecek kejadian
        self.pause = False
        self.FPS = 60
        self.vel = 5
        self.b_vel = 7
        self.WINNER_FONT = pygame.font.SysFont('comicsans', 100)
        self.PAUSE_FONT = pygame.font.SysFont('comicsans', 100)
        self.batas = pygame.Rect(self.width//2 - 5 , 0 , 10, self.height)

        try :
            self.bg = pygame.transform.scale( pygame.image.load(os.path.join('Assets', 'background_2.png')) , (self.width , self.height))
            ...
            print("File asset tidak ditemukan!")
    ...
    def loop (self):
        ...
        while self.run:
            ...
            if self.pause:
                pause_text = self.PAUSE_FONT.render("PAUSED", 1, self.WHITE)
                self.window.blit(pause_text, (self.width/2 - pause_text.get_width() /2, self.height/2 - pause_text.get_height()/2))
                pygame.display.update()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LCTRL and len(self.Player_1.basic_att) < self.peluru:
                    bullet = pygame.Rect(self.Player_1.rect.x + self.Player_1.width_P, self.Player_1.rect.y + self.Player_1.height_P//1.5 -2 , 10, 5)
                    self.Player_1.basic_att.append(bullet)
                if event.key == pygame.K_RCTRL and len(self.Player_2.basic_att) < self.peluru:
                    bullet = pygame.Rect(self.Player_2.rect.x, self.Player_2.rect.y + self.Player_2.height_P//1.5 -2, 10, 5)
    ...
    def login_game(self):
        startimg = pygame.image.load(os.path.join('Assets', 'Start.png'))
        quitimg = pygame.image.load(os.path.join('Assets', 'Quit.png'))
        Hero_1_image_menu = pygame.image.load(os.path.join('Assets', 'MrBLACK.png'))
        Hero_2_image_menu = pygame.image.load(os.path.join('Assets', 'MrRED.png'))
        Hero_3_image_menu = pygame.image.load(os.path.join('Assets', 'MrBRODY.png'))

        self.Start = Menu(startimg, self.width/4, 250, 150, 150)
        self.Quit = Menu(quitimg, self.width*(2/4) + 50, 250, 150, 150)
        MrBLACK = Menu(Hero_1_image_menu, 100, self.height /2 - 50,150, 200)
        MrRED = Menu(Hero_2_image_menu,350, self.height/2 - 50,150, 200 )
        MrBRODY = Menu(Hero_3_image_menu,600, self.height/2 - 50, 150, 200)
        ...
        while self.run == True:
            ...
            if self.Start.display_Menu(self.window):
                self.load_hero(MrBLACK, MrRED, MrBRODY)
                self.loop()
